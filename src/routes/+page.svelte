<script lang="ts">
	...
	const authorize = () => {
		...
		let url = `https://github.com/login/oauth/authorize?client_id=${env.PUBLIC_GH_ID}&redirect_uri=${redirect_uri}&state=${state}&scope=read:repo_hook%20read:user%20read:org`;
